...
instance (Eq l, Eq1 (Cell l)) => Eq2 (Configuration l) where
  liftEq2 eqT eqV (Configuration t1 e1 s1) (Configuration t2 e2 s2) = eqT t1 t2 && liftEq eqV e1 e2 && liftEq eqV s1 s2

instance (Eq l, Eq t, Eq1 (Cell l)) => Eq1 (Configuration l t) where
  liftEq = liftEq2 (==)

instance (Ord l, Ord1 (Cell l)) => Ord2 (Configuration l) where
  liftCompare2 compareT compareV (Configuration t1 e1 s1) (Configuration t2 e2 s2) = compareT t1 t2 <> liftCompare compareV e1 e2 <> liftCompare compareV s1 s2
...
instance (Show l, Show1 (Cell l)) => Show2 (Configuration l) where
  liftShowsPrec2 spT _ spV slV d (Configuration t1 e1 s1) = showsConstructor "Configuration" d [ flip spT t1, flip (liftShowsPrec spV slV) e1, flip (liftShowsPrec spV slV) s1 ]
...
instance (Pretty l, Pretty1 (Cell l)) => Pretty2 (Configuration l) where
  liftPretty2 pT _ pV plV (Configuration t e s) = tupled [ pT t, liftPretty pV plV e, liftPretty pV plV s ]
