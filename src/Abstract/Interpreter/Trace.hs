...
evalTrace :: forall i l. (Monoid (Store l i), AbstractStore l, Context l i (TraceInterpreter l i), AbstractValue i (Eff (TraceInterpreter l i))) => Term i -> (Either String (Val l i, Trace l i []), Store l i)
evalTrace = run @(TraceInterpreter l i) . runTrace

runTrace :: (TraceInterpreter l i :<: fs, AbstractStore l, Context l i fs, AbstractValue i (Eff fs)) => Term i -> Eff fs (Val l i)
runTrace = fix (evTell [] ev)

evalReach :: forall i l. (Monoid (Store l i), Ord i, Ord (l i), Ord (Store l i), AbstractStore l, Context l i (ReachableStateInterpreter l i), AbstractValue i (Eff (ReachableStateInterpreter l i))) => Term i -> (Either String (Val l i, Trace l i Set.Set), Store l i)
evalReach = run @(ReachableStateInterpreter l i) . runReach

runReach :: (Ord i, Ord (l i), Ord (Store l i), ReachableStateInterpreter l i :<: fs, AbstractStore l, Context l i fs, AbstractValue i (Eff fs)) => Term i -> Eff fs (Val l i)
